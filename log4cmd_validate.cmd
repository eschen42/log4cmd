...
:: side effect for RXDQ - double internal quotes
if not RXDQ == %VALIDATOR% goto :not_rxdq
  set SUB1="%UNVALIDATED:"=''%"
  if %SUB1:~0,3%%SUB1:~-3,3% == "''''" set UNVALIDATED="%SUB1:~3,-3%"
...
:validator_check
  ...
  set %1| findstr /i /r /c:"^%1=RXDQ$" /c:"^%1=RXDQNQ$" /c:"^%1=RXNQNS$" >NUL
  exit /b %ERRORLEVEL%
...
:usage
  ...
  echo Usage: %~nx0 ^<result^> {RXDQNQ,RXDQ,RXNQNS} [value] 1>&2
  ...
  echo ^  RXDQ requires that the validand have double quotes at 1>&2
  echo ^    at the first and last character but permits them internally. 1>&2
  echo ^    Side effect: it will convert each double-quote to a pair of single quotes. 1>&2
